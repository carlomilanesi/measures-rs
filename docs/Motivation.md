...
The library Measures is essentially a macro library, and so if you include it in your project, but you don't use it, the measure-handling code is not even generated, and so the compilation of your code has no overhead.

The code generated by Measures grows as long as you add features to it, and however it is quite fast to compile.

For example, the following code uses the library Uom:
```rust
use uom::si::f64::*;
...
fn main() {
    let length = Length::new::<mile>(50.0);
    let time = Time::new::<hour>(2.0);
    let speed = length / time;
    assert_eq!(speed.value, 11.176);
}
```

This is the corresponding code, using the library Measures:
...
measures::define_measure_types! {
    ...
    vector_properties[
        ...
        Speed [
            MeterPerSecond { suffix: " m/s" }
            MilePerHour { suffix: " mi/h", ratio: 1.609344 / 3.6 }
        ]
    ]
    relationships [
        Mile 1 == Hour 1 * MilePerHour 1,
    ]
}

fn main() {
    let length = Measure::<Mile>::new(50.0);
    let time = Measure::<Hour>::new(2.0);
    let speed = (length / time).convert::<MeterPerSecond>();
    assert_eq!(speed.value, 11.176);
}
```

In a particular not-so-powerful machine, the compilation times for these programs have been measured.

The incremental compilation times are the same, at about 0.17 seconds.

Though, the times in seconds for compiling these two programs, after running `cargo clean`, are these:

| Command | Uom | Measures |
|---|---|---|
| `cargo build` | 14.83 | 0.47 |
| `cargo build --release` | 15.95 | 0.39 |

Essentially, full compiles of the program using Measures are 32 to 40 times as fast as the one using Uom.

In addition, the disk space used by Uom in the folder `target` is about ten times as large as the space used by Measures.
...
The value of the variables `position1` and `position2` are meant to be positions of a thing along a line.
The value of the variable `displacement1` is meant to be a movement of that thing along that line.
...
Instead, the value of the variable `what1` is obtained by incrementing position `position1` by position `position2`.
...
```rust
    ...
    // Wrong variable name.
    // This value is actually in miles per hour.
...
Not for every physical or geometrical properties multidimensional measures make sense.
For example, the following statements do not make sense:
```rust
    let angles = Measure3d::<Degree>::new([1., 2., 3.]);
    let times = Measure2d::<Hour>::new([1., 2.]);
```

This is because angles and times are _scalar_ quantities.

So, the library Measures classifies properties into two categories:
- Scalar quantities (like angle, time, mass, temperature, information).
- Vector quantities (like length, velocity, force, electrical field).

Any attempt to declare a multidimensional measure of a scalar quantity causes a compilation error.
