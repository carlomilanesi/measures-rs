...
macro_rules! inner_define_approx_measure {
    { $exact:tt $with_serde:ident } => {
        /// Approximate measure with generic unit of measurement and value type,
        ...
        measures::if_all_true! { { $with_serde }
            impl<Unit, Number> serde::Serialize for ApproxMeasure<Unit, Number>
            where
                Unit: MeasurementUnit,
            ...
            impl<'de, Unit, Number> serde::Deserialize<'de> for ApproxMeasure<Unit, Number>
            where
                Unit: MeasurementUnit,
        ...
        /// format!("{}", ApproxMeasure) -> String
        /// ApproxMeasure.to_string() -> String
