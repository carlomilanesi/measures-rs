...
use units::{Measure, Metre, MilliMetre};
...
fn measure_lossless_into_32_to_32() {
    ...
    let m2: Measure<Metre, f32> = m1.into();
...
fn measure_lossless_into_32_to_64() {
    let m1 = Measure::<Metre, f32>::new(12.);
    let m2: Measure<Metre, f64> = m1.into();
    assert_eq!(m2.value, 12.);
}
...
fn measure_measure_division() {
    ...
    assert_eq!(m3.value, 4.);
}

#[test]
fn measure_one_division() {
    let m1: Measure<Metre, f32> = Measure::<Metre, f32>::new(12.);
    let m2: Measure<One, f32> = Measure::<One, f32>::new(3.);
    let m3: Measure<Metre, f32> = m1 / m2.value;
    assert_eq!(m3.value, 4.);
}

#[test]
fn one_one_multiplication() {
    let m1: Measure<One, f32> = Measure::<One, f32>::new(12.);
    let m2: Measure<One, f32> = Measure::<One, f32>::new(3.);
    let m3: Measure<One, f32> = m1 * m2;
...
#[test]
fn measure_one_multiplication() {
    let m1: Measure<Metre, f32> = Measure::<Metre, f32>::new(12.);
    let m2: Measure<One, f32> = Measure::<One, f32>::new(3.);
    let m3: Measure<Metre, f32> = m1 * m2;
    assert_eq!(m3.value, 36.);
    let m4: Measure<Metre, f32> = m2.value * m1;
